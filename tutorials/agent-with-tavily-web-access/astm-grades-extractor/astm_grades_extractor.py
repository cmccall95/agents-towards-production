...
def search_standard_info(designation: str) -> tuple[Optional[str], Optional[str]]:
    """
    ...
    Returns:
        tuple: (combined_content, error_message) - content is None if failed
    """
...
def process_standard(standard: dict) -> StandardInfo:
    ...
    content, search_error = search_standard_info(designation)
    ...
    if "error" in extracted:
        ...
        logger.info(f"Successfully extracted {designation}: {len(info.grades or [])} grades, {len(info.materials or [])} materials")
